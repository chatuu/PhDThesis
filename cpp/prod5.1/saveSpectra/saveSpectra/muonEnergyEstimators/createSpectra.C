...
void createSpectra()
{
    ...
    //selectionCuts.push_back({"TwoProngNuMuCCAndVertexECut",    kNumuMyQuality && vertexECheck && kIsFiducial && kNumuTightContainND && NuMuCCMuonIDCut && kTwoProng});
    ...
    //interactionCuts.push_back({"RecoNuMuCCIntsNoQE", NuMuCCIntsNoQE});
    //interactionCuts.push_back({"RecoNuMuCCIntsNoQENoDIS", NuMuCCIntsNoQENoDIS});
    //interactionCuts.push_back({"RecoNuMuCCIntsNoQENoDISNoRES", NuMuCCIntsNoQENoDISNoRES});
    //interactionCuts.push_back({"RecoNuMuCCIntsNoQENoDISNoRESNoMEC", NuMuCCIntsNoQENoDISNoRESNoMEC});
    ...
    //vars.push_back({"muonCalENew", {"muonCalENew", Binning::Simple(140, 0, 14), muonCalENew}});
    ...
    //vars.push_back({"True Pion K.E", {"Pion Pion K.E (GeV)", Binning::Simple(3000, 0, 3), kPionProngTrueKE}});
    vars.push_back({"True Muon K.E", {"True #mu^{-} K.E (GeV)", Binning::Simple(3000, 0, 3), kMuonProngTrueKENew}});
    vars.push_back({"Calculated Muon K.E", {"Calculated #mu^{-} K.E (GeV)", Binning::Simple(3000, 0, 3), kMuonProngEstKE}});
    vars.push_back({"Calculated Muon Pt", {"Calculated #mu^{-} Pt (GeV)", Binning::Simple(3000, 0, 3), kMuonPtCalculated}});
    vars.push_back({"Calculated Muon Pl", {"Calculated #mu^{-} Pl (GeV)", Binning::Simple(3000, 0, 3), kMuonPlCalculated}});
    vars.push_back({"True Muon Pt", {"Calculated #mu^{-} Pt (GeV)", Binning::Simple(3000, 0, 3), kTrueMuonPt}});
    vars.push_back({"True Muon Pl", {"Calculated #mu^{-} Pl (GeV)", Binning::Simple(3000, 0, 3), kTrueMuonPl}});
   //vars.push_back({"Reco Muon K.E", {"Estimated #mu^{-} K.E (GeV)", Binning::Simple(3000, 0, 3), kMuonProngEstKE}});
    //vars.push_back({"Muon K.E Resolution", {"#mu^{-} K.E Resolution (#frac{Reco-True}{True})", Binning::Simple(4000, -2, 2), kMuonEResNew}});
    //vars.push_back({"Muon K.E Resolution2", {"#mu^{-} K.E Resolution2 (#frac{Reco-True}{True})", Binning::Simple(4000, -2, 2), kMuonEResNewNoLenLimit}});
    vars.push_back({"Muon K.E ResolutionBPF", {"#mu^{-} K.E Resolution (#frac{Reco-True}{True})", Binning::Simple(4000, -2, 2), kMuonEResBPFNew}});
    vars.push_back({"Muon Pt Resolution", {"#mu^{-} Pt Resolution (#frac{Reco-True}{True})", Binning::Simple(4000, -2, 2), kMuonPtResolution}});
    vars.push_back({"Muon Pl Resolution", {"#mu^{-} Pl Resolution (#frac{Reco-True}{True})", Binning::Simple(4000, -2, 2), kMuonPlResolution}});
    ...
    //vars.push_back({"muonID NuMuCC Inc", {"muonID NumuCC Inc", Binning::Simple(100, 0, 1), muonIDNuMuCCInc}});
    ...
    //vars2D.push_back({"Muon Prong calE (GeV) Vs True Muon Kinetic Energy (GeV)", {"Muon Prong CalE (GeV)", Binning::Simple(300, 0, 3), muonCalENew}, {"True Muon K.E (GeV)", Binning::Simple(300, 0, 3), kMuonProngTrueKENew}});
    ...
    lNDMC.Go();
    //const std::string fname = "defname: prod_caf_R20-11-25-prod5.1reco.a_nd_genie_N1810j0211a_nonswap_fhc_nova_v08_full_v1";
    ...
    //std::vector<const Var *> Newvars = {&kRun, &kSubrun, &kCycle, &kEvt, &kSlice, &kNuE, &vtxX, &vtxY, &vtxZ, &InteractionType, &kMuonProngLenNew, &kMuonProngTrueKENew};
    ...
    TFile *f = new TFile("Feb_19_2021_MuonEnergyEstimatorStudy.root", "recreate");
