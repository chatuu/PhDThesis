...
const Var kPionKEResNew([](const caf::SRProxy *sr){
  ...
  else
  {
	  for (unsigned int i = 0; i < sr->vtx.elastic.fuzzyk.npng; ++i)
	  {
		  if (sr->vtx.elastic.fuzzyk.png[i].spprongcvnpart5label.pionid > pionID)
		  {
...
const Var kPionKEResBPFNew([](const caf::SRProxy *sr){
  ...
  else
  {
    ...
    else
    {
      ...
      if (calE <= 1.1 && calE >=0.1)
	    {
		    double PionKE       = double((sr->vtx.elastic.fuzzyk.png[pionNum].bpf.pion.energy - 0.139570)); 
      	double truePionKE   = double((sr->vtx.elastic.fuzzyk.png[pionNum].bpf.pion.truth.p.E - 0.139570));
        double res = (PionKE - truePionKE)/truePionKE;
        //printf("True: %f Calculated: %f \n", PionKE, truePionKE);
		    return res;
	    }
	    else
